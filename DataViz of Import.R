bigdata_1<-read_csv("/Users/adityabhat/Documents/GEOG788Z/Feb24/Trade_DetailedTradeMatrix_E_All_Data.csv")
...
wheat.reg$incomeLevel.id.x<-droplevels(wheat.reg$incomeLevel.id.x)
wheat.reg$incomeLevel.id.y<-droplevels(wheat.reg$incomeLevel.id.y)
income.x<-factor(wheat.reg$incomeLevel.id.x,levels = c("LIC","LMC","UMC","NOC","OEC"),ordered=T)
income.y<-factor(wheat.reg$incomeLevel.id.y,levels = c("LIC","LMC","UMC","NOC","OEC"),ordered=T)
quantile(income.x,0.25,type=1)
quantile(income.y,0.25,type=1)
